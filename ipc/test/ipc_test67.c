...
void channel67_thread_m(void *ptr)
{
	...
	while (loop-- > 0) {
		...
		do {
			ret = fsl_ipc_recv_ptr(7, &p, &len, ipc_priv);
			usleep(1000);
			tr++;
			if (ret == ERR_SUCCESS)
				tr = 0;
...
void channel67_thread(void *ptr)
{
	...
	printf("range of free pool P=%llx V=%p S=%x\n", r.phys_addr, r.vaddr,
	       r.size);
	ret = fsl_ipc_configure_txreq(6, r.phys_addr + 0x100000, 1024*2, ipc_priv);
	if (ret) {
		printf("\n Error in fsl_ipc_configure_txreq ret %d \n", ret);
		goto end;
	}
	...
	while (1) {
		...
		do {
			if (!first)
				while (fsl_ipc_get_last_tx_req_status(ipc_priv)
				       != TXREQ_DONE) {
						usleep(10000);
						printf(",");
				}
			ret =
			    fsl_ipc_send_tx_req(6, &lst, &fapi_msg, 1020, ipc_priv);
			    usleep(10000);
			    first = 0;
		} while (ret == -ERR_CHANNEL_FULL);
		if (ret) {
...
void test_init(void *id)
{
	...
	err = fsl_ipc_open_prod_ch(6, ipc);
	if (err) {
	...
	if (mute_flag == 1) {
		ret3 = pthread_create(&thread3, NULL,
				(void *)&channel67_thread_m, (void *)ipc_priv);
		if (ret3) {
		...
		ret3 = pthread_create(&thread3, NULL,
				(void *)&channel67_thread, (void *)ipc_priv);
		if (ret3) {
			printf("pthread_create returns with error: %d", ret3);
			return;
		}
